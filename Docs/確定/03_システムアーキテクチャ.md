...
クライアント（Flutter）はUIとセンシングに専念し、複雑な意思決定は Backend 上の **Google ADK** が担います。
...
flowchart TB
    ...
    subgraph APIs["🌍 External APIs"]
        ...
        PoliceData["警視庁統計<br/>(Firestore + Geohash)"]
        RoutesAPI["Google Routes API"]
        StreetView["Street View API<br/>(Static)"]
        SolarAPI["Solar API<br/>(暗さ・影判定)"]
        PlacesAPI["Places API (New)<br/>(避難所・コンビニ)"]
    end
...
クロスプラットフォームでiOS/Android両対応のユーザーインターフェースを提供します。
- **google_maps_flutter**: 地図描画・ポリライン表示を担当。
- **Provider / Riverpod**: 状態管理。
- **HTTP Client (dio)**: Backend API との通信。
...
- **言語**: Python
- **AI**: **Vertex AI (Gemini)** 経由で呼び出し
- **デプロイ**: Cloud Run（コンテナベース）
- **Input Agent**: 複数の外部APIから「現在の状況（天気、災害リスク、治安）」を収集し、統合します。
- **Risk Evaluator (Gemini)**: Vertex AI の **Gemini** モデルを使用し、収集したデータから「今の状況でこの道を通るべきか？」を人間のように推論します。
