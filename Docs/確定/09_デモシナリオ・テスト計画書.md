...
- **Action**: 「駅から自宅へ」をセット。
- **AI Voice**: 「お疲れ様です。今日は少し遅いので、人通りの多い明るい道（公園通り）を案内しますね。」
...
- **Visual**: 突然のアラート音。APIモックで「ゲリラ豪雨（Rain: 80mm/h）」を投入。
- **Action**: ユーザーが「非常時モード（赤）」スイッチをON。
- **UI Change**: 画面が一瞬で赤黒くなり、Thinking Logが高速で流れる。
  ```
  > WARNING: HEAVY RAIN DETECTED.
...
flowchart TD
    ...
    subgraph Integration [結合テスト]
        I1[iOS → Backend<br/>End-to-End疎通]
...
### Unit Test
- Route Agentが「全ルート不可」の時に正しくWaypointを生成するか？（Mockで強制的にFailさせてテスト）
- Vision APIがタイムアウトした時にUIがフリーズしないか？

### Field Test (Simulation)
- GPSシミュレータ（Xcode）を使用し、渋谷駅周辺を移動するシミュレーションを行う。
